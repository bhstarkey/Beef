...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var selectedSample = samples.filter(selectedID => selectedID.id === sample);
    // console.log(selectedSample);

    // Deliverable 3: 1. Create a variable that filters the metadata array for the object with the desired sample number.
    var volunteerData = data.metadata;
    var selectedVolunteer = volunteerData.filter(sampleObj => sampleObj.id == sample);
    ...
    var washFreq = parseFloat(firstVolunteer.wfreq);
